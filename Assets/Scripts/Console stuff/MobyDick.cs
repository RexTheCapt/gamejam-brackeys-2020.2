...
internal class MobyDick
{
    public static string GetLine(int lineNr, int linesBack)
    {
        StringBuilder sb = new StringBuilder();

        for (int i = lineNr - linesBack; i <= lineNr; i++)
        {
            if (i < 0)
                continue;
            sb.Append($"{lines[i % lines.Length]}\n");
        }

        return sb.ToString();
    }

    // This is no joke the entire moby dick book
